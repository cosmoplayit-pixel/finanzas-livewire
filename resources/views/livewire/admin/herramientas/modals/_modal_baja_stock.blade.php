    ...
    <x-ui.modal wire:key="baja-stock-modal" model="openBajaStockModal" title="Baja de Stock" maxWidth="sm:max-w-xl"
        ...
        <div class="space-y-4">
            ...
            @php
                $cantidadBaja = $bajaStockTipo === 'activo'
            ...
            <div class="grid grid-cols-2 gap-2">
                <div
                    ...
                    <div class="text-[10px] uppercase font-bold text-gray-400 dark:text-neutral-500 mb-1">Stock actual</div>
                ...
                <div
                    ...
                    <div class="text-[10px] uppercase font-bold text-red-500 dark:text-red-400 mb-1">Stock Restante</div>
                    <div class="text-2xl font-black text-red-600 dark:text-red-400 font-mono">
                        {{ max(0, $bajaStockActual - $cantidadBaja) }}</div>
                </div>
            </div>

            @if ($bajaStockTipo === 'activo')
                ...
                <div>
                    ...
                    @if (empty($bajaStockSeriesDisponibles))
                        <div class="rounded-lg border border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-800 px-4 py-3 text-xs text-gray-500 dark:text-neutral-400 text-center">
                            Sin series disponibles para dar de baja.
                        </div>
                    @else
                        <div class="rounded-xl border border-gray-200 dark:border-neutral-700 overflow-hidden divide-y divide-gray-100 dark:divide-neutral-800 max-h-48 overflow-y-auto">
                            @foreach ($bajaStockSeriesDisponibles as $s)
                                <label class="flex items-center gap-3 px-4 py-2.5 cursor-pointer hover:bg-red-50 dark:hover:bg-red-900/10 transition {{ in_array($s['id'], $bajaStockSeriesSeleccionadas) ? 'bg-red-50 dark:bg-red-900/10' : 'bg-white dark:bg-neutral-900' }}">
                                    ...
                                    <span class="font-mono text-sm font-semibold text-gray-800 dark:text-neutral-200">{{ $s['serie'] }}</span>
                                </label>
                            @endforeach
                        </div>
                    @endif
        ...
        @slot('footer')
            <div class="w-full grid grid-cols-2 gap-2">
                ...
                <button type="button" wire:click="saveBajaStock" wire:loading.attr="disabled"
                    @disabled(!$bajaStockObservaciones || ($bajaStockTipo === 'activo' ? empty($bajaStockSeriesSeleccionadas) : (!$bajaStockCantidad || $bajaStockCantidad <= 0)))
