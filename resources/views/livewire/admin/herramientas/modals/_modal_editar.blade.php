    ...
    <x-ui.modal wire:key="edit-herramienta-modal" model="editModal" title="Editar Herramienta"
        ...
        <div class="space-y-4">
            ...
            <div
                ...
                <span>Estás editando un registro existente. Podés modificar todos los datos incluido el stock.</span>
            </div>
            ...
            <div class="grid grid-cols-2 lg:grid-cols-3 gap-3">
                ...
                <div class="col-span-1" x-data="{
                    ...
                    get suggestions() {
                        if (!this.query || !this.query.trim()) return this.unidades.slice(0, 10);
                    ...
                    <div class="relative">
                        ...
                        <div x-show="open && suggestions.length > 0" x-cloak
                            ...
                            <template x-for="u in suggestions" :key="u.value">
                ...
                <div class="col-span-1">
                    ...
                    <input type="number" wire:model.live="stock_disponible" min="0" disabled
                        class="w-full rounded-lg border px-3 py-2 bg-gray-100 dark:bg-neutral-800 border-gray-300 dark:border-neutral-700 text-gray-500 dark:text-neutral-400 focus:outline-none focus:ring-2 focus:ring-indigo-500/40 font-bold text-center text-sm opacity-75">
                    ...
                    @if ($editDeleteImagen && !$imagen)
                        <div class="mt-1.5 flex items-center gap-1.5 text-xs text-red-600 dark:text-red-400">
                            ...
                            <button type="button" wire:click="$set('editDeleteImagen', false)"
                                class="underline cursor-pointer ml-1">Cancelar</button>
                        </div>
                    @endif
                </div>
                ...
                <div class="p-3 grid grid-cols-2 gap-4">
                    <div class="space-y-0.5">
                        ...
                        <div class="text-sm font-black text-emerald-600 dark:text-emerald-400">
                            {{ $stock_disponible }} {{ $unidad ?: 'Unid.' }}</div>
                    </div>
                    <div class="space-y-0.5 text-right">
                        ...
                        <div class="text-sm font-black text-gray-900 dark:text-neutral-100">Bs.
                            {{ number_format(($stock_disponible + $stock_prestado) * (float) $precio_unitario, 2, ',', '.') }}
                        </div>
                    </div>
                </div>
            </div>
        ...
        @slot('footer')
            <div class="w-full grid grid-cols-2 gap-2 sm:flex sm:justify-end sm:gap-3">
                ...
                <button type="button" wire:click="update" wire:loading.attr="disabled" @disabled(!$nombre || !$precio_unitario)
                    class="w-full sm:w-auto px-6 py-2 rounded-lg cursor-pointer bg-amber-600 text-white hover:bg-amber-700 transition text-sm font-black shadow-lg shadow-amber-600/20 disabled:opacity-50 disabled:cursor-not-allowed">
