            ...
            <table class="w-full text-[13px] min-w-[1200px] align-middle">
                ...
                <tbody class="divide-y divide-gray-200 dark:divide-neutral-700">
                    @forelse($movimientos as $m)
                        @php
                            $isTargetMovPrivado =
                                isset($highlight_movimiento_id) && $highlight_movimiento_id == $m['id'];
                        @endphp
                        <tr @if ($isTargetMovPrivado) id="mov-tgt-{{ $m['id'] }}" @endif
                            class="{{ $isTargetMovPrivado ? 'bg-amber-50/60 dark:bg-amber-900/20 border-l-4 border-amber-400' : 'hover:bg-gray-50 dark:hover:bg-neutral-900/50' }} text-center align-middle font-extralight transition-colors">
