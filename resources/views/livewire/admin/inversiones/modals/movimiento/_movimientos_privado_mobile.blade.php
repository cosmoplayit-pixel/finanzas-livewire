    ...
    <div class="rounded-2xl border border-gray-200 bg-white overflow-hidden dark:border-neutral-700 dark:bg-neutral-900/30"
            ...
            @forelse($movimientos as $m)
                @php
                    ...
                    if ($tipo === 'CAPITAL_INICIAL') {
                        ...
                        $badgeText = $isPend ? ($isVencido ? 'VENCIDO' : 'PENDIENTE') : 'PAGADO';
                    ...
                    if ($tipo === 'PAGO_UTILIDAD') {
                        if ($isPend) {
                            ...
                            $badge = $badgeBase . ' bg-sky-100 text-sky-700 dark:bg-sky-900/30 dark:text-sky-300';
                        }
                    } elseif ($tipo === 'INGRESO_CAPITAL') {
                        $badge =
                            $badgeBase .
                        ...
                        $badge =
                            $badgeBase . ' bg-purple-100 te xt-purple-700 dark:bg-purple-900/30 dark:text-purple-300';
                    }
                    ...
                    if ($tipo === 'CAPITAL_INICIAL') {
                        ...
                        if ($isPend) {
                            if ($isVencido) {
                                $bgGrad =
                                    'bg-gradient-to-b from-rose-50/80 to-white ' .
                                    'dark:from-rose-900/15 dark:to-neutral-900/10';
                            } else {
                                $bgGrad =
                        ...
                        // purple tenue
                        $bgGrad =
                            'bg-gradient-to-b from-purple-50/80 to-white ' .
                            'dark:from-purple-900/15 dark:to-neutral-900/10';
                    }
                @endphp

                @php
                    $isTargetMovPrivadoMob = isset($highlight_movimiento_id) && $highlight_movimiento_id == $m['id'];
                    $borderHighlight = $isTargetMovPrivadoMob
                        ? 'border-amber-400 bg-amber-50/40 dark:bg-amber-900/10'
                        : 'border-gray-200 dark:border-neutral-700 ' . $bgGrad;
                @endphp

                <div @if ($isTargetMovPrivadoMob) id="mov-mob-tgt-{{ $m['id'] }}" @endif
        ...
        <div class="border-t border-gray-200 dark:border-neutral-700 bg-gray-50 dark:bg-neutral-900">
            <div class="px-3 py-2  space-y-2">
                ...
                {{-- Vencidos --}}
                <div
                    class="rounded-2xl border border-rose-200 bg-white p-3 dark:border-rose-800/40 dark:bg-neutral-900/40">
                    <div class="flex items-center justify-between">
                        <div
                            class="inline-flex items-center gap-2 text-[9px] font-bold tracking-wide uppercase text-rose-800 dark:text-rose-200">
                    ...
                    <div class="mt-2 grid grid-cols-3 gap-2 text-[11px]">
                        ...
                        <div class="rounded-xl bg-gray-50 px-3 py-2 dark:bg-neutral-900/60">
                            <div class="text-[9px] uppercase tracking-wide text-gray-500 dark:text-neutral-400">Último
                                %</div>
                            <div class="mt-1 font-semibold tabular-nums text-rose-900 dark:text-rose-200">
                                {{ $tVen['lastPctFmt'] ?? '—' }}
                            </div>
                        </div>
                    </div>
                ...
                <div
                    class="hidden rounded-2xl border border-amber-200 bg-white p-3 dark:border-amber-800/40 dark:bg-neutral-900/40">
                    <div class="flex items-center justify-between">
                        <div
